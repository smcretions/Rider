...
class NetworkProvider {
  ...
  static Future<PolylineResult> getRouteBetweenCoordinates(
    ...
    try {
      final response = await _dio
          .getUri(
            uri,
            options: dio.Options(headers: _getDirectionsHeaders()),
          )
          .timeout(timeout ?? _defaultTimeout);

      if (response.statusCode == 200) {
        return _parseDirectionsResponse(response.data);
      } else {
        return PolylineResult.error(
          'Failed to fetch route: ${response.statusCode} - ${response.statusMessage}',
        );
      }
    } catch (e) {
      if (e is dio.DioException) {
        return PolylineResult.error('Network error: ${e.message}');
      }
  ...
  static Future<RoutesApiResponse> getRouteBetweenCoordinatesV2(
    ...
    try {
      final response = await _dio
          .post(
            _routesBaseUrl,
            options: dio.Options(headers: _getRoutesHeaders(googleApiKey, request)),
            data: request.toJson(),
          )
          .timeout(timeout ?? _defaultTimeout);
      ...
      if (response.statusCode == 200) {
        ...
        // v2 api does not include status so use statusMessage instead
        data['status'] = response.statusMessage;
        return RoutesApiResponse.fromJson(data);
      } else {
        final errorData = response.data;
        return RoutesApiResponse.error('Routes API error: ${response.statusCode} - ${errorData['error']?['message'] ?? response.statusMessage}');
      }
    } catch (e) {
      if (e is dio.DioException) {
        return RoutesApiResponse.error('Network error: ${e.message}');
      }
      throw Exception('Network error: ${e.toString()}');
    }
  }
  ...
  static Uri _buildDirectionsUri(String? apiKey, PolylineRequest request) {
    ...
    if (request.wayPoints.isNotEmpty) {
      final waypoints = request.wayPoints.map((wp) => '${wp.stopOver ? '' : 'via:'}${wp.location}').join('|');
